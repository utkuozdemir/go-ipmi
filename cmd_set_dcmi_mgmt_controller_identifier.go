...
func (c *Client) SetDCMIMgmtControllerIdentifier(offset uint8, writeBytes uint8, idStr []byte) (response *SetDCMIMgmtControllerIdentifierResponse, err error) {
...
func (c *Client) SetDCMIMgmtControllerIdentifierFull(idStr []byte) (err error) {
	if len(idStr) > 63 {
	...
	for {
		...
		_, err := c.SetDCMIMgmtControllerIdentifier(offset, writeBytes, idStr[offset:offsetEnd])
		if err != nil {
