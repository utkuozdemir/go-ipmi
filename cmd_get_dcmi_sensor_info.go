...
func (c *Client) GetDCMISensorInfo(request *GetDCMISensorInfoRequest) (response *GetDCMISensorInfoResponse, err error) {
	response = &GetDCMISensorInfoResponse{}
	err = c.Exchange(request, response)
	return
}

func (c *Client) GetDCMISensors(entityIDs ...EntityID) ([]*SDR, error) {
	out := make([]*SDR, 0)
	...
	for _, entityID := range entityIDs {
		...
		for _, recordID := range response.SDRRecordID {
			sdr, err := c.GetSDREnhanced(recordID)
			if err != nil {
