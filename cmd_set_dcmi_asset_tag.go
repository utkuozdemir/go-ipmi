...
func (c *Client) SetDCMIAssetTag(offset uint8, writeBytes uint8, assetTag []byte) (response *SetDCMIAssetTagResponse, err error) {
...
func (c *Client) SetDCMIAssetTagFull(assetTag []byte) (err error) {
	if len(assetTag) > 63 {
	...
	for {
		...
		_, err := c.SetDCMIAssetTag(offset, writeBytes, assetTag[offset:offsetEnd])
		if err != nil {
