...
// This file implements a second server frontend, parallel to the RMCP+ UDP
// [Server], that speaks QEMU's OpenIPMI VM protocol (the ipmi-bmc-extern wire
// format). It exists so an emulated machine's in-band system interface and an
// out-of-band LAN client can drive one shared [bmc.BMC], which is what the
// library's simulation / end-to-end use targets: a guest kernel and a userland
// agent talk to the BMC over the VM protocol while tooling talks to the same
// BMC over LAN, and both observe the same state.
//
...
// messages dispatch with a bare [handlers.HandlerContext].
...
const (
	...
	vmCmdVersion      = 0xFF // QEMU announces the protocol version on connect
	vmCmdCapabilities = 0x08 // QEMU announces its capabilities on connect
...
func (s *VMServer) serveConn(ctx context.Context, conn net.Conn) {
	...
	// The VM protocol is the in-band system interface (channel 0x0F). Attribute
	// dispatched requests to it so "this channel" (0x0E) resolves to the system
	// interface rather than falling back to LAN. A missing channel leaves the
	// context channel nil and downstream handlers fall back gracefully.
...
// handleControl processes a single-byte control command from QEMU (the version
// and capability announcements sent on connect). These carry no BMC state, and
// the BMC never sends hardware control commands back: power authority stays
// with the hardware layer, so unknown control commands are ignored.
func (s *VMServer) handleControl(_ []byte) {}
