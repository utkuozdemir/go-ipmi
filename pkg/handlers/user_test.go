...
func TestHandleGetUserAccessEnumerable(t *testing.T) {
	...
	_, cc, _ := handleGetUserAccess(ctx, hctx, []byte{0x01, b.Users.MaxUserCount() + 1})
...
func TestHandleSetUserPassword(t *testing.T) {
	...
	// Test Password matches for both the 16-byte and 20-byte forms of the same
	// secret (the store pads to 20).
	...
	if _, cc, _ := handleSetUserPassword(ctx, hctx, setPasswordReq(7, passwordOpTestPassword, true, secret)); cc != types.CodeOK {
		t.Errorf("test 20-byte cc = 0x%02x, want OK", cc)
	}

	// Wrong password fails with the command-specific 0x80.
	if _, cc, _ := handleSetUserPassword(ctx, hctx, setPasswordReq(7, passwordOpTestPassword, false, "wrong")); cc != codePasswordTestFailed {
...
func TestHandleSetUserPassword20ByteNoPrefixMatch(t *testing.T) {
	...
	// A 16-byte Test carrying only the first 16 bytes must fail.
	if _, cc, _ := handleSetUserPassword(ctx, hctx, setPasswordReq(8, passwordOpTestPassword, false, full20[:16])); cc != codePasswordTestFailed {
		t.Errorf("16-byte prefix test cc = 0x%02x, want 0x80 (fail)", cc)
	}
