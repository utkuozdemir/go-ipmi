...
// Transport (LAN) command bytes referenced by the privilege table.
const (
	NetFnTransportRequest uint8 = 0x0c
	CmdGetLanConfigParam  uint8 = 0x02
)
...
func handleGetLanConfigParam(ctx context.Context, hctx *HandlerContext, req []byte) ([]byte, types.CompletionCode, error) {
	...
	// Validate the parameter selector before honoring a revision-only query, so
	// revision-only for an unsupported selector still returns the
	// parameter-not-supported code rather than a spurious success.
	if !supportedLanParam(param) {
	...
	case types.LanConfigParamSelector_SetInProgress:
		...
		return lanParamResponse(byte(types.SetInProgress_SetComplete)), types.CodeOK, nil
	...
	case types.LanConfigParamSelector_IP,
		...
		return handleLanAddressParam(ctx, hctx, param)
...
// handleLanAddressParam answers the address-family LAN parameters from the
...
func handleLanAddressParam(ctx context.Context, hctx *HandlerContext, param types.LanConfigParamSelector) ([]byte, types.CompletionCode, error) {
	...
	switch param {
	case types.LanConfigParamSelector_IP:
		return lanParamResponse(cfg.IP[:]...), types.CodeOK, nil
	...
	case types.LanConfigParamSelector_IPSource:
		...
		return lanParamResponse(byte(source)), types.CodeOK, nil

	case types.LanConfigParamSelector_MAC:
		return lanParamResponse(cfg.MAC[:]...), types.CodeOK, nil

	case types.LanConfigParamSelector_SubnetMask:
		return lanParamResponse(cfg.Mask[:]...), types.CodeOK, nil

	case types.LanConfigParamSelector_DefaultGatewayIP:
		return lanParamResponse(cfg.Gateway[:]...), types.CodeOK, nil
...
// supportedLanParam reports whether the reference server implements the given
// LAN configuration parameter selector. Address-family parameters are listed
// even when no NIC is present; the handler then reports the command as
// unsupported for the missing NIC, which is distinct from an unknown selector.
func supportedLanParam(param types.LanConfigParamSelector) bool {
	switch param {
	case types.LanConfigParamSelector_SetInProgress,
		types.LanConfigParamSelector_AuthTypeSupport,
		types.LanConfigParamSelector_PrimaryRMCPPort,
